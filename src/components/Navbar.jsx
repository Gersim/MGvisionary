...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-background/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16">
                    <div className="flex-shrink-0 flex items-center cursor-pointer">
                        <Link to="hero" smooth={true} duration={500} className="text-2xl font-heading font-bold text-primary tracking-tight">
                        ...
                        <div className="ml-10 flex items-baseline space-x-8">
                            {navLinks.map((link) => (
                                <Link
                                    ...
                                    smooth={true}
                                    duration={500}
                                    className="text-text hover:text-accent cursor-pointer px-3 py-2 rounded-md text-sm font-medium transition-colors relative group"
                                >
                                    {link.name}
            ...
            {/* Mobile menu */}
            {isOpen && (
                <div className="md:hidden bg-background shadow-lg absolute w-full left-0">
